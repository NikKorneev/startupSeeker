...
// Query: *[_type == "startup" && _id == $startupId][0] {      _id,     title,    slug,    _createdAt,    author -> {      _id, name,image,bio, username    },    views,description,category,    image,    pitch    }
export type STARTUP_QUERYResult = {
  _id: string;
  title: string | null;
  slug: Slug | null;
  _createdAt: string;
...
// Variable: STARTUP_VIEWS
// Query: *[_type == "startup" && _id == $startupId][0] {  views, _id}
export type STARTUP_VIEWSResult = {
  views: number | null;
  _id: string;
} | null;
...
// Query: *[_type == "playlist" && slug.current == $slug][0] {      _id,         title,        slug,        select[]->{          _id,          title,          _createdAt,          image,          description,          slug,          author->{            _id,            name,            slug,            image,            bio          }        },        views,        description,        category,        image,        pitch      }
